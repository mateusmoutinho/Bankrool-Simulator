...
- `schema_validator.py` — Validates game JSON configs. Enforces two game schemas: multi-bet-type and single-bet-type. In both types, total probability cannot exceed 1.0 and losing is implicit from remaining probability.
...
- `config/games/` — Game definitions. Two types distinguished by the `multi-bet-type` boolean:
  - **Multi-bet** (e.g., Baccarat): has `bet-types` dict where each bet type contains a `possible-results` list. Each result has `name`, `multiplier`, and `chance`. Loss is implicit from remaining probability.
  - **Single-bet** (e.g., Slots, MTT): has a top-level `possible-results` list with the same result format. Loss is implicit from remaining probability.

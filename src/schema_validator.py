...
def _validate_multi_bet_game(game: dict) -> None:
    ...
    for bet_name, bet_config in bet_types.items():
        ...
        # Validate multiplier
        if "multiplier" not in bet_config:
            raise ValueError(f"Game '{game_name}' bet type '{bet_name}' must have 'multiplier' field")
        
        multiplier = bet_config["multiplier"]
        if not isinstance(multiplier, (int, float)):
            raise ValueError(f"Game '{game_name}' bet type '{bet_name}' multiplier must be a number")
        
        if multiplier <= 0:
            raise ValueError(f"Game '{game_name}' bet type '{bet_name}' multiplier must be positive")
        
        # Validate chance
        if "chance" not in bet_config:
            raise ValueError(f"Game '{game_name}' bet type '{bet_name}' must have 'chance' field")
        
        chance = bet_config["chance"]
        if not isinstance(chance, (int, float)):
            raise ValueError(f"Game '{game_name}' bet type '{bet_name}' chance must be a number")
        
        if not 0 <= chance <= 1:
            raise ValueError(f"Game '{game_name}' bet type '{bet_name}' chance must be between 0 and 1")
        
        total_chance += chance
    
    # Validate total probability (allow small floating point errors)
    if not (0.99 <= total_chance <= 1.01):
        raise ValueError(
            f"Game '{game_name}' total chance across all bet types must sum to ~1.0 "
